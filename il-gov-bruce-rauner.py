...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=50):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['chris kennedy', 'rauner', 'IL gubernatorial', '@govrauner', '^(?!.*bissexual).*biss.*$', 'illinois governor', 'IL\'s next governor ', 'governor of illinois', 'il gov', 'jeanne ives', 'Welcome to the dysfunction of Illinois government', 'Gubernatorial Forum Sunday', 'High school students to host gubernatorial primary debate', 'rage at Rauner', 'Rauner Vetoes Geolocation Privacy Protection Act', 'unpaid bill backlog hits a record', 'pritzker', 'il governor\'s']
            for term in terms:
