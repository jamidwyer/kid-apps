...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=100):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['^(?!.*west virginia governor).*virginia governor.*$', 'mcauliffe', 'Ralph northam', 'va gov race', 'VA gov candidate', 'Virginia needs a governor', 'Trump operatives zero in on these key races in Virginia', 'Made Your Democratic Vote Useless', 'Northam responding to a Trump tweet', 'final Virginia Pride appearance', 'Democratic Candidate Leads by 10 in Virginia Gubernatorial', 'va. gubernatorial race', 'gillespe', 'Virginia\'s race for governor', 'virginia gubernatorial']
            for term in terms:
