...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=100):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['^(?!.*west virginia governor).*virginia governor.*$', 'debate spotlights Democratic problems in rural America', 'Having a hard time deciding who to vote for on Nov 7', 'Latino group launches anti-Trump Virginia campaign ad', 'northam and gillespie', 'mcauliffe', 'ed gillespie', 'va gov race', 'VA gov candidate', 'Virginia needs a governor', 'shockingly racist ads', 'Trump operatives zero in on these key races in Virginia', 'va. gubernatorial race', 'gillespe', 'Virginia\'s race for governor', 'virginia gubernatorial']
            for term in terms:
