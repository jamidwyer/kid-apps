...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=100):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['^(?!.*west virginia governor).*virginia governor.*$', 'Latino group launches anti-Trump Virginia campaign ad', 'northam and gillespie', 'mcauliffe', 'Ralph northam', 'va gov race', 'VA gov candidate', 'Virginia needs a governor', 'shockingly racist ads', 'Trump operatives zero in on these key races in Virginia', 'Made Your Democratic Vote Useless', 'Northam responding to a Trump tweet', 'va. gubernatorial race', 'gillespe', 'Virginia\'s race for governor', 'virginia gubernatorial']
            for term in terms:
