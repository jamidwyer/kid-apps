...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=50):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['Black women are the backbone of the Democratic Party', 'signal boosting black female candidates for elected office', 'want to live in a kleptocratic plutocracy', 'jeffares' 'buddy carter', 'lisaringga', 'lisa ring', 'reason for Republicans to start panicking about 2018', 'Republican congressman from the state wants to withhold federal dollars to test these kits', 'withhold federal funding to test rape kits', '@repbuddycarter', 'Senate opponents of Trumpcare should be beaten', 'another round of anger over health care', 'fantasizes about beating up a female Republican Senator', 'snatch a knot']
            for term in terms:
