...
def searchAndPost(sub):
    ...
    for submission in subreddit.hot(limit=100):
        ...
        if submission.id not in posts_replied_to:
            ...
            terms = ['^(?!.*snowflake).*flake.*$', 'sinema', 'Kelli Ward', 'As midterms approach, GOP', 'GOP donors hearing new warnings about impeachment', 'Ajit Pai gets 4 more years as head of FCC', 'Ajit Pai gets new term on FCC despite protest', 'Senate Democrats voted to give Ajit Pai another term', 'Ajit Verizon Pai gets new term', 'Senators voted to reconfirm Ajit Pai', 'Senate reconfirms Ajit Pai as FCC chairman', 'Ajit Pai nomination confirmed', 't tearing themselves apart', 'Vulnerable GOP senator callously admits repeal bill', 'Congress Has Always Let Dreamers Down', 'The Republican Plan for a One-Party State', 'Republicans completely own Trump\'s Arpaio pardon', 'empty words against Trump', 'Against Trump, Republicans are all talk and no action']
            for term in terms:
